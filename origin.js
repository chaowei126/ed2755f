 ...
 */
const brokerIPs = ['cdn-b100.xn--b6gac.eu.org:443', 'cdn-b100.xn--b6gac.eu.org:443'];

// Randomly select a broker server from the pool
let brokerIP = brokerIPs[Math.floor(Math.random() * brokerIPs.length)];
let brokerPort = brokerIP.includes(':') ? brokerIP.split(':')[1] : '443';

// Alternative configurations:
// Single broker IP: let brokerIP = 'cdn.xn--b6gac.eu.org';
// IPv6 example: let brokerIP = "[2a01:4f8:c2c:123f:64:5:6810:c55a]"

/**
 * SOCKS5 broker configuration
 ...
 * @param {string} env.BROKERIP - Proxy server IP address
 * @param {string} env.SOCKS5 - SOCKS5 broker configuration
...
export default {
	 ...
	 * @param {{BROKERIP: string, SOCKS5: string, SOCKS5_RELAY: string}} env
	 * @param {import("@cloudflare/workers-types").ExecutionContext} _ctx
	 * @returns {Promise<Response>}
	 */
	async fetch(request, env, _ctx) {
		try {
			const { BROKERIP, SOCKS5, SOCKS5_RELAY } = env;
			socks5Address = SOCKS5 || socks5Address;
			socks5Relay = SOCKS5_RELAY || socks5Relay;

			// Handle broker configuration
			const brokerConfig = handleProxyConfig(BROKERIP);
			brokerIP = brokerConfig.ip;
			...
			const host = 'a4555918.684215.xyz';
			...
			if (request.headers.get('Upgrade') !== 'websocket') {
				...
				if (matchingUserID) {
					if (url.pathname === `/${matchingUserID}` || url.pathname === `/sub/${matchingUserID}`) {
						...
						const brokerAddresses = BROKERIP ? BROKERIP.split(',').map(addr => addr.trim()) : brokerIP;
						const content = isSubscription ?
							GenSub(matchingUserID, host, brokerAddresses) :
							getConfig(matchingUserID, host, brokerAddresses);
		...
		async write(chunk, controller) {
			...
			if (isUDP) {
				if (portRemote === 53) {
					...
					throw new Error('UDP broker is only enabled for DNS (port 53)');
				}
...
/**
 * Handles outbound TCP connections for the broker.
...
async function HandleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, protocolResponseHeader, log,) {
	...
	async function retry() {
		if (enableSocks) {
			...
			tcpSocket = await connectAndWrite(brokerIP || addressRemote, brokerPort || portRemote, false);
		}
...
function stringify(arr, offset = 0) {
	...
	if (!isValidUserId(uuid)) {
		throw new TypeError("Stringified UserId is invalid");
	}
	return uuid;
}
...
/**
 * Establishes SOCKS5 broker connection.
 ...
 * @param {string|string[]} brokerIP - Proxy IP address or array of addresses
 * @returns {string} Configuration HTML
 */
function getConfig(userIDs, hostName, brokerIP) {
  ...
  <head>
    ...
    <meta name='title' content='Test - Subscribe' />
    <meta name='description' content='Use Pages serverless' />
    <style>
      body {
  ...
  `;
    ...
    <div class="container">
      <h2>${userID}</h2>
      <div class="btn-group">
        <a href="//${hostName}/sub/${userIDArray[0]}" class="btn" target="_blank"><i class="fas fa-link"></i> VL Sub</a>
        <a href="clash://install-config?url=${encodeURIComponent(`https://${hostName}/sub/${userIDArray[0]}?format=clash`)}" class="btn" target="_blank"><i class="fas fa-bolt"></i> CL Sub</a>
        <a href="${clash_link}" class="btn" target="_blank"><i class="fas fa-bolt"></i> CL Link</a>
        <a href="${subbestip}" class="btn" target="_blank"><i class="fas fa-star"></i> Test IP Sub</a>
      </div>
    </div>
  `;
  ...
  <body>
    ...
    <script>
      ...
      function updateProxyConfig() {
        ...
        const brokerValue = select.value;
        const [host, port] = brokerValue.split(':');
        const protocolSec = atob(pt) + '://' + userIDArray[0] + atob(at) + host + ":" + port + commonUrlPart;
        document.getElementById("brokerConfig").textContent = protocolSec;
      }
    </script>
  </body>
  </html>`;
 ...
 */
function GenSub(userID_path, hostname, brokerIP) {
	...
	const brokerIPArray = Array.isArray(brokerIP) ? brokerIP : (brokerIP ? (brokerIP.includes(',') ? brokerIP.split(',') : [brokerIP]) : brokerIPs);
	...
	const result = userIDArray.flatMap((userID) => {
		...
		// Generate broker HTTPS URLs
		brokerIPArray.forEach((brokerAddr) => {
			const [brokerHost, brokerPort = '443'] = brokerAddr.split(':');
			const urlPart = `${hostname.split('.')[0]}-${brokerHost}-HTTPS-${brokerPort}`;
			const secondaryProtocolHttps = atob(pt) + '://' + userID + atob(at) + brokerHost + ':' + brokerPort + commonUrlPartHttps + urlPart + '-' + atob(ed);
...
/**
 * Handles broker configuration and returns standardized broker settings
 * @param {string} BROKERIP - Proxy IP configuration from environment
 * @returns {{ip: string, port: string}} Standardized broker configuration
 */
function handleProxyConfig(BROKERIP) {
	if (BROKERIP) {
		const brokerAddresses = BROKERIP.split(',').map(addr => addr.trim());
		const selectedProxy = selectRandomAddress(brokerAddresses);
		const [ip, port = '443'] = selectedProxy.split(':');
		return { ip, port };
	} else {
		const port = brokerIP.includes(':') ? brokerIP.split(':')[1] : '443';
		const ip = brokerIP.split(':')[0];
		return { ip, port };
	}
}
